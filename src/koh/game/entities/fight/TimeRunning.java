...
/* TODO MASK
        * 22,Ne pas tenter de retirer de PM aux adversaires pendant toute la durée du combat.
        */
public class TimeRunning extends Challenge{
    public TimeRunning(Fight fight, FightTeam team) {
        super(fight, team);
    }
    ...
    public void onFighterCastSpell(Fighter fighter, SpellLevel spell) {
        if(Arrays.stream(spell.getEffects()).anyMatch(e-> e.getEffectType() == StatsEnum.SUB_PA
                || e.getEffectType() == StatsEnum.SUB_PA_ESQUIVE
                || e.getEffectType() == StatsEnum.SUB_PA_ESQUIVE_2)){
            this.failChallenge();
        }
    }
