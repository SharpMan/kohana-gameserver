            ...
            try (ConnectionResult conn = dbSource.executeQuery("SELECT * FROM `character` WHERE owner = '" + account.id + "';", 0)) {
                ...
                while (result.next()) {
                    ...
                    p = Player.builder()
                            ...
                            .vitality(Integer.parseInt(stats[0]))
                            .wisdom(Integer.parseInt(stats[1]))
                            .strength(Integer.parseInt(stats[2]))
                            .intell(Integer.parseInt(stats[3]))
                            .agility(Integer.parseInt(stats[4]))
                            .chance(Integer.parseInt(stats[5]))
                            .life(Integer.parseInt(stats[6]))
                            .experience(Long.parseLong(stats[7]))
                            .activableTitle(Short.parseShort(stats[8]))
                            .activableOrnament(Short.parseShort(stats[9]))
                            .regenStartTime(Long.parseLong(stats[10]))
                            .koliseoPoints(Integer.parseInt(stats[11]))
